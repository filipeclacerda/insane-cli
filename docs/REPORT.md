...
| Sandbox e tools | `tests/tools_sandbox.rs` (novo) | 19 | escapes de sandbox (`..`, absoluto fora do cwd, symlink -- este último roda quando o SO permite criar symlink sem privilégio elevado, senão pula com aviso) tanto em `read_file`/`list_files`/`edit_file`; `.env` limpo é permitido, mas scanner de segredos em `.env`/`read_file` nega sem vazar a chave fake; `edit_file` único/ambíguo/`replace_all`/não encontrado (prova que a lógica de match está correta até o portão de confirmação -- ver limitação abaixo); `run_command` negado nunca cria o arquivo que o comando criaria; `list_files` respeita `.gitignore` (inclusive padrão de diretório, ver bug abaixo) e o cap de 500; `search_files` cap de 100 e resultado customizado |
